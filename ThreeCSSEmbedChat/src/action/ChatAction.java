...
public class ChatAction {
	public static List<Chat> getChatList(int toType, String toTypeId, int chatType, Date chatCreateTime) {
		...
		try {
			...
			LogManager.mariadbLog.error("获取聊天异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static Chat createChat(String chatContent, int toType, String toTypeId, String fromUserId) {
		...
		try {
			...
			if (result == 0) {
				LogManager.mariadbLog.warn("创建聊天内容失败");
				return null;
			}
			sqlSession.commit();
			return chat;
			...
			LogManager.mariadbLog.error("创建聊天内容异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static Chat getChatById(String chatId) {
		...
		try {
			...
			if (chat == null) {
				LogManager.mariadbLog.warn("通过chatId:" + chatId + "获取聊天内容为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
			}
			LogManager.mariadbLog.error("获取聊天内容异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static boolean updateChat(List<String> chatIdList) {
		...
		try {
			...
			if (result == 0) {
				LogManager.mariadbLog.warn("修改用户聊天内容失败");
				return false;
			}
			if (result != chatIdList.size()) {
				LogManager.mariadbLog.warn("修改用户聊天内容失败,提交" + chatIdList.size() + "条，修改" + result + "条");
			}
			sqlSession.commit();
		} catch (Exception e) {
			if (sqlSession != null) {
			...
			LogManager.mariadbLog.error("修改用户聊天内容异常", e);
			return false;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();

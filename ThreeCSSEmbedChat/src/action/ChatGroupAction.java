...
public class ChatGroupAction {
	public static List<ChatGroup> getChatGroupListByUserId(String userId) {
		...
		try {
			...
			if (chatGroupList == null) {
				LogManager.mariadbLog.warn("通过userId:" + userId + "获取聊天组为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
	...
	public static List<ChatGroup> getChatGroupList(String userGroupId) {
		...
		try {
			...
			if (chatGroupList == null) {
				LogManager.mariadbLog.warn("通过userGroupId:" + userGroupId + "获取聊天组为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
	...
	public static ChatGroup createChatGroup(String chatGroupName, String userGroupId) {
		...
		try {
			...
			if (result == 0) {
				LogManager.mariadbLog.warn("创建聊天组失败");
				return null;
			}
			sqlSession.commit();
			return chatGroup;
			...
			LogManager.mariadbLog.error("创建聊天组异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();

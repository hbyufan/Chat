...
public class ChatGroupUserAction {
	public static List<ChatGroupUser> getChatGroupUserList(String userId, String chatGroupId) {
		...
		try {
			...
			if (chatGroupUserList == null) {
				LogManager.mariadbLog.warn("通过userId:" + userId + ",chatGroupId:" + chatGroupId + "获取聊天组为空");
			}
		} catch (Exception e) {
			if (sqlSession != null) {
				sqlSession.rollback();
			}
			LogManager.mariadbLog.error("获取聊天组异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static ChatGroupUser updateChatGroupUser(String userId, String chatGroupId, Date chatCreateTime) {
		...
		try {
			...
			if (result != 1) {
				LogManager.mariadbLog.warn("修改聊天组用户失败");
				return null;
			}
			sqlSession.commit();
		} catch (Exception e) {
			...
			LogManager.mariadbLog.error("修改聊天组用户异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
	...
	public static ChatGroupUser createChatGroupUser(String userId, String chatGroupId, String chatGroupUserRealName, int chatGroupUserRole) {
		...
		try {
			...
			if (result == 0) {
				LogManager.mariadbLog.warn("创建聊天组用户失败");
				return null;
			}
			sqlSession.commit();
			...
			LogManager.mariadbLog.error("创建聊天组用户异常", e);
			return null;
		} finally {
			if (sqlSession != null) {
				sqlSession.close();
